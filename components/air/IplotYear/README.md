...
This script takes a input CSV file with daily pollen samples and plot a graph for a specific pollen (`output.html`). The input CSV format is as follows:
...
```sh
docker build -t $REGISTRY/enbic2lab/air/iplot_year:1.0.0 .
```
...
```sh
docker run --rm $REGISTRY/enbic2lab/air/iplot_year:1.0.0 --help
```

e.g.

```sh
docker run --rm -v $(pwd)/data:/mnt/shared/ $REGISTRY/enbic2lab/air/iplot_year:1.0.0 --filepath /mnt/shared/munich.csv --pollen Alnus
```
