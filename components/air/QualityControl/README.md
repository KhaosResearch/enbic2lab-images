...
This script takes a input CSV file with daily pollen samples and perform a quality control that generates a PDF file (`output.pdf`) and another CSV file (`output.csv`). The input CSV format is as follows:
...
```sh
docker build -t $REGISTRY/enbic2lab/air/quality_control:1.0.0 .
```
...
```sh
docker run --rm $REGISTRY/enbic2lab/air/quality_control:1.0.0 --help
```

e.g.

```sh
docker run --rm -v $(pwd)/data:/mnt/shared/ $REGISTRY/enbic2lab/air/quality_control:1.0.0 --filepath /mnt/shared/munich.csv
```
