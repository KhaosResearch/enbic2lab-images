...
def __prepare_date_dataframe(df, df_filter, start_date, end_date):
    ...
    date = df.iloc[int(first_element) : int(last_element) + 1,]["FECHA"]
    df_filter["FECHA"] = date
    ...
    if (
        ...
        or datetime.strptime(df_filter.iloc[0,]["FECHA"], "%Y-%m-%d")
        > start_date_datetime
    ) or (
        datetime.strptime(df_filter.iloc[-1,]["FECHA"], "%Y-%m-%d") < end_date_datetime
    ):
        raise ValueError(
            f"Enter a valid range of dates. Values are stored between {df_filter.iloc[0,]['FECHA']} and {df_filter.iloc[-1,]['FECHA']}."
        )

    df_filter = df_filter[
        (df_filter["FECHA"] >= start_date) & (df_filter["FECHA"] <= end_date)
    ]
...
def __perform_linear_regression(
    ...
    Args:
        ...
        target_station (str): The name of the target station for analysis.
        analysis_stations (List[str]): A list of station names for analysis.
        series_completion (pd.DataFrame): The DataFrame containing FECHA and series data.
    ...
    series_completion = series_completion.set_index("FECHA")
    ...
    for station in analysis_stations:
        ...
        completion = df[["FECHA", station]].dropna()
        completion = completion.set_index("FECHA")
...
def __complete_target_station(
    ...
    """
    target_completion = filtered_df[["FECHA", target_station]]
    target_completion = target_completion.set_index("FECHA")
...
def series_completion(
    ...
    series_completion = pd.DataFrame(columns=["FECHA"])
    series_completion["FECHA"] = df_filter["FECHA"]
