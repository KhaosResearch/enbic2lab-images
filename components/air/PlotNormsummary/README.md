...
```sh
docker build -t $REGISTRY/enbic2lab/air/plot_normsummary:1.0.0 .
```
...
```sh
docker run --rm $REGISTRY/enbic2lab/air/plot_normsummary:1.0.0 --help
```

e.g.

```sh
docker run --rm -v $(pwd)/data:/mnt/shared/ $REGISTRY/enbic2lab/air/plot_normsummary:1.0.0 --filepath /mnt/shared/munich.csv --pollen Alnus
```