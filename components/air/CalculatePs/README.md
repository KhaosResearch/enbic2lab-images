...
This script takes a input CSV file with daily pollen samples and calculate the main parameters of the pollen season with regard to phenology and pollen
intensity. This generates a PDF file (`output.pdf`) and another XLSX (`output.xlsx`). The input CSV format is as follows:
...
```sh
docker build -t $REGISTRY/enbic2lab/air/calculate_ps:1.0.0 .
```
...
```sh
docker run --rm $REGISTRY/enbic2lab/air/calculate_ps:1.0.0 --help
```

e.g.

```sh
docker run --rm -v $(pwd)/data:/mnt/shared/ $REGISTRY/enbic2lab/air/calculate_ps:1.0.0 --filepath /mnt/shared/munich.csv --method percentage
```
