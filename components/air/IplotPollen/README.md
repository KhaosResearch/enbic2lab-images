...
This script takes a input CSV file with daily pollen samples and plot a graph for a specific year (`output.html`). The input CSV format is as follows:
...
```sh
docker build -t $REGISTRY/enbic2lab/air/iplot_pollen:1.0.0 .
```
...
```sh
docker run --rm $REGISTRY/enbic2lab/air/iplot_pollen:1.0.0 --help
```

e.g.

```sh
docker run --rm -v $(pwd)/data:/mnt/shared/ $REGISTRY/enbic2lab/air/iplot_pollen:1.0.0 --filepath /mnt/shared/munich.csv --year 2010
```
