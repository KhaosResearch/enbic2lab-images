...
This script takes a input CSV file with daily pollen samples and plots a graphic with pollen grains / m^3 for a specific pollen type during a specific year (`output.pdf`). The input CSV format is as follows:
...
```sh
docker build -t $REGISTRY/enbic2lab/air/plot_ps:1.0.0 .
```
...
```sh
docker run --rm $REGISTRY/enbic2lab/air/plot_ps:1.0.0 --help
```

e.g.

```sh
docker run --rm -v $(pwd)/data:/mnt/shared/ $REGISTRY/enbic2lab/air/plot_ps:1.0.0 --filepath /mnt/shared/munich.csv --pollenType Alnus --year 2010
```